...
router.get('/parking_area/filter/:operator/:capacity', async (req, res) => {
    const { operator, capacity } = req.params
    try {
        const data = await getFilteredParkingAreas(operator, capacity)
        res.send(data)
    } catch (e) {
        console.error(e)
    }
